...
@section('title', 'Edit Closer')

@section('content')
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Closer: {{ $closer->name }}</h1>

    <form action="{{ route('closers.update', $closer) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6">
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" required value="{{ old('name', $closer->name) }}"
            ...
            <div class="mb-4">
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $closer->email) }}"
                    class="mt-1 block w-full px-4 py-2.5 rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-gray-900">
                @error('email')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
        ...
        <div class="bg-white shadow rounded-lg p-6 mb-6" x-data="{ search: '' }">
            ...
            <p class="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-md p-3 mb-4">
                <strong>Note:</strong> Assigning a client to a closer does not update existing charges. Commission calculations are based on the current client-closer assignment and will only affect future commission views. Historical charge records remain unchanged.
            </p>
                ...
                >
                    <label class="flex items-center">
                        <input type="checkbox" name="clients[{{ $client->id }}][assigned]" value="1"
                            {{ $closer->clients->contains($client->id) ? 'checked' : '' }}
                    ...
                    <input type="number" step="0.01" min="0" max="100" 
                        ...
                        value="{{ $closer->clients->contains($client->id) ? ($closer->clients->find($client->id)->pivot->commission_rate ?? 0) : 0 }}"
        ...
        <div class="flex justify-end">
            <a href="{{ route('closers.index') }}" class="mr-3 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                Cancel
            </a>
